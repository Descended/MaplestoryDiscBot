...
from src.Main import DB_SETTINGS
...
class DatabaseHandler:
    ...
    def get_character_stats(character_name):
        try:
            # con = mysql.connector.connect(host=Config.DATABASE_HOST, user=Config.DATABASE_USER, password=Config.DATABASE_PASS, database=Config.DATABASE_NAME)
            con = mysql.connector.connect(host=DB_SETTINGS['HOST'], user=DB_SETTINGS['NAME'],
                                          password=DB_SETTINGS['USER'], database=DB_SETTINGS['PASS'])
